---
layout: post
title:  XAP Videos 
categories:
parent: none
---


{%comment%}

<br>

{%section%}

{%column width=20% %}
[Whats new](#1) <br>
{%endcolumn%}

{%column width=20% %}
[XAP Product](#2)<br>
[Installation](#3)
{%endcolumn%}

{%column width=20% %}
[The XAP Interface](#4)<br>
[Persistence](#5)<br>
[Processing](#6)
{%endcolumn%}

{%column width=20% %}
[Web Application Support](#7) <br>
[Wan and Multi Site Replication](#8) <br>
[Administration](#9)
{%endcolumn%}

{%column width=20% %}
[Big Data](#1) <br>
[Interoperability](#10) <br>
[Other Data Access APIs](#11)
{%endcolumn%}

{%endsection%}

{%endcomment%}

<br>

#### Click a thumbnail to watch the relevant video

<br>


{%anchor 1%}

### Whats new

{%section%}

{%column width=20% %}
{%youtube gRdGWMigJBI | Global HTTP Session sharing%}
{%endcolumn%}

{%column width=20% %}
{%youtube fgXdQhPkXBQ | XAP ASP NET Session-Store%}
{%endcolumn%}


{%column width=20% %}
{%youtube kAe-ZxFnIYc | XAP MemoryXtend%}
{%endcolumn%}

{%column width=20% %}
{%youtube H22qPux8Fh8 | Aggregation%}
{%endcolumn%}

{%endsection%}


<br>

{%anchor 2%}

### XAP Product

{%section%}
{%column width=20% %}
{%youtube ZI8Q0FPxcGM | GigaSpaces XAP In-Memory Computing Platform %}
{%endcolumn%}


{%column width=20% %}
{%youtube P0rTANBRuWE | GigaSpaces XAP In-Memory Computing Platform %}
{%endcolumn%}

{%endsection%}

<br>


{%anchor 3%}

### Installation

{%section%}
{%column width=20% %}
{%youtube Q1zCFxqbqrQ | Installing XAP%}
{%endcolumn%}

{%column width=20% %}
{%youtube _n8ZCnoAIyw | Maven Plugin%}
{%endcolumn%}

{%column width=20% %}
{%youtube I2s8jg_Gufs | Hello World Application%}
{%endcolumn%}



{%endsection%}



<br>

{%anchor 4%}

### XAP Interface



{%section%}
{%column width=20% %}
{%youtube nBljYC3ciYc | Document API %}
{%endcolumn%}

{%column width=20% %}
{%youtube jC57mId3SMg | XAP Query APIS%}
{%endcolumn%}


{%column width=20% %}
{%youtube k2zGdUi_ntU | Custom Change API%}
{%endcolumn%}

{%column width=20% %}
{%youtube H22qPux8Fh8 | Aggregation%}
{%endcolumn%}

{%endsection%}

<br>

{%anchor 5%}

### Persistence

{%section%}
{%column width=20% %}
{%youtube DlVohFGdqg8 | Async Persistence %}
{%endcolumn%}


{%endsection%}

{%anchor 6%}

###   Processing

{%section%}

{%column width=20% %}
{%youtube n7P4rnQN1gw | Map Reduce%}
{%endcolumn%}

{%column width=20% %}
{%youtube GwLfDYgl6f8 | Event Processing%}
{%endcolumn%}

{%column width=20% %}
{%youtube psjo9212zC8 | Event Processing%}
{%endcolumn%}


{%column width=20% %}
{%youtube -07-0PXUoeM | Space based remoting%}
{%endcolumn%}

{%endsection%}


<br>

{%anchor 7%}

### Big Data

{%section%}

{%column width=20% %}
{%youtube ioHwEsARPWI | Real-Time Analytics for Big Data%}
{%endcolumn%}

{%column width=20% %}
{%youtube c9L06fKbbW8 | XAP Storm Integration%}
{%endcolumn%}


{%endsection%}


<br>

{%anchor 8%}

### Web Application Support

{%section%}
{%column width=20% %}
{%youtube YTEqFzrfVss | Web Application support %}
{%endcolumn%}

{%column width=20% %}
{%youtube gRdGWMigJBI | Global HTTP Session sharing%}
{%endcolumn%}

{%column width=20% %}
{%youtube fgXdQhPkXBQ | XAP ASP NET Session-Store%}
{%endcolumn%}

{%endsection%}


<br>

{%anchor 9%}

### Wan and Multi Site Replication

{%section%}
{%column width=20% %}
{%youtube wRtTpWYebg0  | WAN Replication Deployment and Management Automation%}
{%endcolumn%}

{%column width=20% %}
{%youtube 0ArjYgLJADc  | XAP WAN Bridge %}
{%endcolumn%}

{%column width=20% %}
{%youtube V7rbbmWo3JU | Scalable Data Replication Across Sites %}
{%endcolumn%}


{%endsection%}




<br>

{%anchor 10%}

### Administration and Management

{%section%}
{%column width=20% %}
{%youtube 9nFkfj2o5hk | XAP UI%}
{%endcolumn%}


{%column width=20% %}
{%youtube j3FfHBqtj1o | Admin API%}
{%endcolumn%}

{%column width=20% %}
{%youtube rb5LWyj68Zk | Monitoring%}
{%endcolumn%}


{%column width=20% %}
{%youtube usWQ0tqRoq0 | Administrative Events %}
{%endcolumn%}


{%column width=20% %}
{%youtube 2jI6Z1z4UrU | Administrative Events updated%}
{%endcolumn%}

{%endsection%}


<br>

{%anchor 11%}

### Interoperability

{%section%}
{%column width=20% %}
{%youtube ZTRvuE8kQs0 | Java and .Net Interoperability %}
{%endcolumn%}


{%endsection%}

<br>

{%anchor 12%}

### Other Data Access APIs

{%section%}
{%column width=20% %}
{%youtube YRbzC_89G5Q | XAP's JPA implementation %}
{%endcolumn%}


{%endsection%}
